...
describe('generating', () => {
  ...
  it('can be waited on, so a caller that tears down its directory does not race the write', async () => {
    // The key poll calls ensure() without awaiting it, so a stop can return with a
    // rename still in flight. Aboard that lands harmlessly; where the directory is
    // temporary it lands on nothing, and the rejection has nowhere to go - it took
    // down a CI run in which every test had already passed, pointing at a file the
    // change had not touched. flush() is what stop() awaits.
    const store = reload()
    void store.ensure()
    await store.flush()
    fs.rmSync(dir, { recursive: true, force: true })
    // Nothing left queued: a second flush resolves with the directory already gone.
    await store.flush()
