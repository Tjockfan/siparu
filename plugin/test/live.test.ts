...
/**
 * The rule channel: five reads and one write share this socket, and only the write has to
 * prove who sent it.
 *
 * What is pinned here is the wiring, not the proof - the proof is measured next door in
 * alertrules.test.ts. What matters at this layer is which answers leave the boat and, more
 * importantly, which do not.
 */
describe('the alert rule channel', () => {
  const RULES = { rules: [{ path: 'notifications.tanks.bilge.0', ring_from: 'never' as const }], ts: 7, known: [] }

  /** Answers as the asking screen holds them: opened out of the envelope she sealed. */
  const repliesOf = (s: FakeSocket, type: string): Array<Record<string, unknown>> =>
    s
      .frames()
  ...
  it('answers a read with the list she is going by', async () => {
    const { live, last } = uplink({ onAlertRulesQuery: async () => RULES })
    live.start()
    last().open()
    last().ask({ type: 'alertrules', id: 'q1' })
    await flush()

    expect(repliesOf(last(), 'alertrules')).toEqual([{ type: 'alertrules', id: 'q1', result: RULES }])
  })

  it('carries a write to the handler and sends back what it answered', async () => {
    const seen: string[] = []
    const { live, last } = uplink({
      onSetAlertRules: async (req) => {
        seen.push(req.kid)
        return { type: 'setalertrules', id: req.id, result: { rules: [], ts: 9 } }
      }
    })
    live.start()
    last().open()
    ...
    expect(seen).toEqual(['kid-phone'])
    expect(repliesOf(last(), 'setalertrules')).toEqual([
      { type: 'setalertrules', id: 'w1', result: { rules: [], ts: 9 } }
    ])
  })

  it('says nothing at all to a write the handler would not vouch for', async () => {
    // The silence is the feature. Her inbox key is public, so an unproven write is a stranger,
    // and a refusal would confirm to him that he had found a boat.
    const { live, last } = uplink({ onSetAlertRules: async () => undefined })
    live.start()
    last().open()
    const before = last().sent.length
    last().ask({
  ...
  it('does not reach the handler with a message that is not shaped like a write', async () => {
    const seen: unknown[] = []
    const { live, last } = uplink({ onSetAlertRules: async (r) => (seen.push(r), undefined) })
    live.start()
    last().open()
    for (const bad of [
      { type: 'setalertrules', v: 1, id: 'w', kid: 'k', ts: 1, rules: 'all', proof: 'x' },
      { type: 'setalertrules', v: 1, id: 'w', kid: 'k', ts: 1.5, rules: [], proof: 'x' },
      { type: 'setalertrules', v: 1, id: '', kid: 'k', ts: 1, rules: [], proof: 'x' },
      'not json at all'
    ]) {
      if (typeof bad === 'string') last().say(bad)
      else last().ask(bad)
    }
    await flush()
  ...
  it('refuses a read that arrives in the clear', async () => {
    // A screen that asked the old way stops waiting and can say why. An encrypted boat is not
    // a broken one, and silence here would read as the latter. The bare request is deliberate:
    // this is the one case in the file that still puts one on the wire.
    const { live, last } = uplink({
      onAlertRulesQuery: async () => RULES
    })
    live.start()
    last().open()
    last().say(JSON.stringify({ type: 'alertrules', id: 'q1' }))
    await flush()

    const [refusal] = repliesOf(last(), 'alertrules')
    expect((refusal.error as { code: string }).code).toBe('SEALED')
  })

  it('says nothing to a WRITE that arrives in the clear while she is sealing', async () => {
    // Deliberately not refused, unlike the read beside it. No screen writes a list in the
    // clear - the app seals its questions whenever the boat is sealing - so a cleartext write
    // to a sealed boat is not a screen at all, and there is nobody waiting to be released.
    const called: unknown[] = []
    const { live, last } = uplink({
      onSetAlertRules: async (r) => (called.push(r), undefined)
    })
    live.start()
    last().open()
    const before = last().sent.length
  ...
  it('opens a sealed write and answers it sealed', async () => {
    // The path a real screen uses: the write arrives inside an envelope, is dispatched through
    // the same guards as everything else, and the answer goes back sealed or not at all.
    const { live, last } = uplink({
      openAsk: (env) => {
        const e = env as { body?: string }
        return e.body ? { id: 'w1', plaintext: e.body } : undefined
      },
      sealAnswer: (payload, id) => ({ mode: 'sealed', frame: { sealedAnswer: payload, id } }),
      onSetAlertRules: async (req) => ({
        type: 'setalertrules',
        id: req.id,
        result: { rules: [], ts: 9 }
    ...
    const [answer] = repliesOf(last(), 'answer')
    expect(answer.id).toBe('w1')
    expect((answer.frame as { sealedAnswer: { type: string } }).sealedAnswer.type).toBe(
      'setalertrules'
    )
  })
})
