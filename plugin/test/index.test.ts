...
function fakeApp(dataDir: string) {
  const calls = {
    ...
    unsubscribed: 0,
    errors: [] as string[],
    savedOptions: [] as object[],
    /** Every path she asked the server for, so what she is deaf to is visible. */
  ...
  const app = {
    ...
    subscriptionmanager: {
      subscribe: (cmd: unknown, unsubscribes: Array<() => void>) => {
        calls.subscribes++
        for (const s of (cmd as SubscribeCommand).subscribe ?? []) {
          if (typeof s.path === 'string') calls.subscribed.push(s.path)
        }
        unsubscribes.push(() => {
          calls.unsubscribed++
        })
