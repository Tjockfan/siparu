...
describe('VoyageLog', () => {
  ...
  // The two slowest fixture tests in the suite, and both for the same reason: the row-by-row
  // loop runs about 10x slower under QEMU armv7, where it has crossed the default ceiling.
  // They get their own. The suite-wide ceiling still guards everything else.
  //
  // The second one crossed it later than the first, and what pushed it over was not a change to
  // voyages at all: it sat a fraction under 30s on that runner, and a slice of new tests
  // elsewhere in the suite was enough to lose the fraction. That is worth naming rather than
  // fixing quietly, because the next test to arrive here will be a third one.
