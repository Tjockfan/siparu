...
describe('pair/approve', () => {
  ...
  it('names the plan when the account has no room for her, instead of "try again"', async () => {
    // Same shape as the refusal above and for the same reason: an account that already holds
    // every boat its plan allows will hold them tomorrow too. What is different is where the
    // skipper has to go to fix it, so the message has to say so rather than blaming the link.
    let saved: RemoteState | undefined | 'untouched' = 'untouched'
    vi.stubGlobal(
      'fetch',
      vi.fn(async (url: string) =>
        String(url).endsWith('/pair/approve')
          ? new Response(JSON.stringify({ error: 'boat_limit' }), { status: 409 })
          : new Response(
              JSON.stringify({ device_code: 'dc', user_code: 'WDJB-MJHT', expires_in: 3600 }),
              { status: 200 }
            )
    ...
    expect(body.message).toMatch(/as many boats as its plan allows/i)
    expect(body.message).not.toMatch(/try again\./i)
    expect(body.message).not.toMatch(/does not own this boat/i)
