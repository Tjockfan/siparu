...
/**
 * The exclusion an allow-list guard hands its second grep, as a RegExp.
 *
 * These guards are shaped `grep <writes> | grep -v <the ones we allow>`, so what
 * matters is not what the first pattern catches but what the second lets past. That
 * list grows every time a write route is added, and it is exactly the kind of edit
 * that goes in one character too wide: `/voyages/` instead of the two actions, and
 * every future write on that resource walks through green.
 */
function allowList(stepName: string, marker: string): RegExp {
  const yml = readFileSync(CI_YML, 'utf8')
  const step = yml.split(/^      - name: /m).find((s) => s.startsWith(stepName))
  if (!step) throw new Error(`no CI step named "${stepName}" - was it renamed or removed?`)
  const m = new RegExp(`grep -vE "(.+?)"\\) \\|\\| true`).exec(step)
  if (!m) throw new Error(`step "${stepName}" no longer excludes anything, so it allows everything`)
  if (!m[1]!.includes(marker)) throw new Error(`step "${stepName}" exclusion does not mention ${marker}`)
...
describe('the write routes the plugin is allowed to have', () => {
  const allowed = allowList('GET-only REST', 'merge-previous')

  it.each([
    "router.post('/pair/start', handler)",
  ...
  ])('lets through %j, which exists and is accounted for', (line) => {
    expect(allowed.test(line)).toBe(true)
  })

  it.each([
    ["router.post('/voyages/:id/delete', handler)", 'a third verb on the same resource'],
    ["router.post('/voyages/:id/set-ports', handler)", 'another edit nobody reviewed'],
    ["router.put('/voyages/:id/merge-previous', handler)", 'the same route by another method'],
    ["router.post('/config/anything-else', handler)", 'a second config write'],
    ["router.delete('/pair/start', handler)", 'a pairing route turned destructive']
  ])('stops %j (%s)', (line) => {
    expect(allowed.test(line)).toBe(false)
  })
})

describe('the write calls the webapp is allowed to make', () => {
  const allowed = allowList('Read-only proof - the webapp writes only', 'merge-previous')

  it.each([
    "  start: () => http<PairScreen>('/pair/start', { method: 'POST' }),",
    "  setFuelPaths: (paths) => http('/config/fuel-paths', { method: 'POST', body: x })",
    '  mergePrevious: (id: number) => http(`/voyages/${id}/merge-previous`, { method: \'POST\' }),',
    '  undoMerge: (id: number) => http(`/voyages/${id}/undo-merge`, { method: \'POST\' }),'
  ])('lets through %j', (line) => {
    expect(allowed.test(line)).toBe(true)
  })

  it.each([
    ['  wipe: (id: number) => http(`/voyages/${id}/delete`, { method: \'POST\' })', 'a delete dressed as an edit'],
    ["  setState: () => http('/signalk/v1/api/vessels/self/navigation/state', { method: 'PUT' })", 'a write to the vessel'],
    ['  rename: (id: number) => http(`/voyages/${id}`, { method: \'PATCH\' })', 'a patch on the resource itself']
  ])('stops %j (%s)', (line) => {
    expect(allowed.test(line)).toBe(false)
  })
})
