...
describe('system readings', () => {
  ...
  it('reads fuel.economy, which the schema does not publish and boats report anyway', () => {
    // Measured on a boat running Signal K's derived-data plugin, which computes this path as
    // speed / rate and so delivers metres per cubic metre. 1654554.71 m/m3 is 1.12 L/nm, and the
    // owner's own separately derived L/nm figure on the same instant read 1.12. Before this the
    // path matched nothing, fell through to the plain-number fallback, and put "1654554.7" in an
    // engine cell.
    expect(systemValue('propulsion.engine.fuel.economy', 1654554.71)).toBe('1.12 L/nm')
    expect(systemValue('propulsion.port.fuel.economy', 1848605.3)).toBe('1.00 L/nm')
    // A thirsty motor yacht, the reading the inverse would have shown as "0.04 nm/L".
    expect(systemValue('propulsion.starboard.fuel.economy', 74080)).toBe('25.00 L/nm')

    const cell = describePath('propulsion.engine.fuel.economy')
    expect(cell?.tab).toBe('engine')
    expect(cell?.sub).toBe('Fuel per mile')

    const band = systemNumeric('propulsion.engine.fuel.economy', 1654554.71)
    expect(band.value).toBeCloseTo(1.11933, 5)
    expect(band.unit).toBe('L/nm')
  })

  it('prints no economy at rest, where the reciprocal is an infinity', () => {
    // An engine idling at the dock: rate > 0, speed 0, so derived-data publishes an economy of
    // exactly 0. Inverted that is Infinity, and "Infinity L/nm" in an engine cell is worse than
    // the raw number this replaced. The cell says it cannot read, the same dot every unreadable
    // value gets, and the graph door answers NaN so a chart drops the sample instead of scaling
    // its whole axis to a spike.
    expect(systemValue('propulsion.engine.fuel.economy', 0)).toBe('·')
    expect(Number.isNaN(systemNumeric('propulsion.engine.fuel.economy', 0).value)).toBe(true)
    // The axis still knows what it plots with no usable reading in hand.
    expect(systemNumeric('propulsion.engine.fuel.economy', 0).unit).toBe('L/nm')
  })
  ...
  it('subs no burn rate as an economy, so an L/nm cell cannot collide with one', () => {
    ...
    for (const path of [
      ...
      // The L/nm cell itself. It is the one path entitled to the word and does not take it: a
      // person reads economy as distance per volume, and this cell shows the reciprocal.
