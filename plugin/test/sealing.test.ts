...
describe('sealing a question to the boat', () => {
  const inbox = generateKeyPairSync('x25519')
  const inboxPub = rawPublic(inbox.publicKey)
  const inboxPriv = rawPrivate(inbox.privateKey)
  const ask = (id = 'h1', boat = 'boat-0001', text = '{"type":"voyages","id":"h1","limit":5}') =>
    sealRequest({ boat, id, plaintext: text, inbox: inboxPub })

  it('carries the id in the clear and nothing else', () => {
    const sealed = ask()
    // The relay routes the answer and counts what is outstanding by this id. A question it
    // could not read the id of is a question whose answer has nowhere to go.
    expect(sealed.id).toBe('h1')
    expect(Object.keys(sealed).sort()).toEqual(['body', 'eph', 'id', 'type', 'v'])
    expect(JSON.stringify(sealed)).not.toContain('voyages')
  })

  it('is opened by the boat it was addressed to', () => {
    const opened = openRequest(ask(), 'boat-0001', inboxPriv, inboxPub)
    expect(opened.id).toBe('h1')
    expect(JSON.parse(opened.plaintext).type).toBe('voyages')
  })

  it('refuses a question addressed to another vessel', () => {
    // The boat is bound into the derivation rather than carried, so a question captured on the
    // way to one vessel is not a question at another. Nothing has to be compared for this to
    // hold: the wrong boat simply derives the wrong key.
    expect(() => openRequest(ask(), 'boat-0002', inboxPriv, inboxPub)).toThrow()
  })

  it('refuses a question whose id was rewritten in transit', () => {
    // The id is cleartext and the relay reads it, so it has to be bound to the body somehow.
    // It is in the derivation, which means rewriting it costs the attacker the plaintext.
    const sealed = { ...ask(), id: 'h2' }
    expect(() => openRequest(sealed, 'boat-0001', inboxPriv, inboxPub)).toThrow()
  })

  it('refuses a body that was altered', () => {
    const sealed = ask()
    const bytes = Buffer.from(sealed.body, 'base64url')
    bytes[0] ^= 0x01
    expect(() =>
      openRequest({ ...sealed, body: bytes.toString('base64url') }, 'boat-0001', inboxPriv, inboxPub)
    ).toThrow()
  })

  it('refuses a question sealed to somebody else', () => {
    const stranger = generateKeyPairSync('x25519')
    const sealed = sealRequest({
      boat: 'boat-0001',
      id: 'h1',
      plaintext: '{}',
      inbox: rawPublic(stranger.publicKey)
    })
    expect(() => openRequest(sealed, 'boat-0001', inboxPriv, inboxPub)).toThrow()
  })

  it('refuses malformed envelopes rather than reading them', () => {
    const junk: unknown[] = [
      null,
      'a string',
      { type: 'ask', v: 2, id: 'h1', eph: '', body: '' },
      { type: 'ask', v: 1, id: '', eph: '', body: '' },
      { type: 'ask', v: 1, id: 'h1', eph: 'not base64url!!', body: '' },
      { type: 'ask', v: 1, id: 'h1', eph: b64uOf(32), body: '' },
      { type: 'ask', v: 1, id: 'h1' },
      // A frame is not a question, whatever else it carries.
      { v: 1, id: 'h1', eph: b64uOf(32), body: b64uOf(20) }
    ]
    for (const bad of junk) {
      expect(() => openRequest(bad, 'boat-0001', inboxPriv, inboxPub)).toThrow()
    }
  })

  it('never repeats an ephemeral key, so two identical questions differ on the wire', () => {
    const first = ask()
    const second = ask()
    expect(first.eph).not.toBe(second.eph)
    expect(first.body).not.toBe(second.body)
  })
})

/** A base64url string of n zero bytes, for envelopes that are shaped right and mean nothing. */
function b64uOf(n: number): string {
  return Buffer.alloc(n).toString('base64url')
}
