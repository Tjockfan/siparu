 ...
 *
 * plugin/test/fixtures/ pins its output on a synthetic multi-leg track.
 ...
 *
 * `manual` names voyages whose boundaries an owner set himself. This pass runs
 * on every appended snapshot, so without it a hand-made decision lasts until the
 * next one arrives: two voyages joined by hand meet the automatic test for being
 * joined to whatever follows, and two put back apart meet it for being joined to
 * each other again, at a gap of zero and a hop of zero. Neither side of a pair
 * is touched if either is marked, because a merge changes both.
 */
...
export async function reconcile(
  ...
  state.voyages = await mergeContiguousVoyages(
    state.voyages,
    opts,
    async (v) => {
