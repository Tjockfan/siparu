...
export class BoatKeyStore {
  ...
  /**
   * Wait until a queued write has landed on disk.
   *
   * `ensure()` is called from a poll the plugin does not await, so a stop can
   * return with a rename still in flight. Aboard that is harmless; the write
   * finishes into a directory that is still there. Under test the directory is
   * a temporary one that is removed the moment the case ends, and the rename
   * then fails into nothing that is waiting for it - an unhandled rejection
   * that fails the run after every test has passed, with a stack pointing at a
   * file nobody touched. stop() awaits this.
   */
  flush(): Promise<void> {
