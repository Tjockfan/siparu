 ...
 */
export function strictDecode(value: unknown, expectedBytes?: number): Buffer {
 ...
 *
 * These four are exported because a second set of bytes two implementations
 * have to agree on byte for byte is built from them (the rule proof, 3.8). A
 * second copy of them elsewhere would be a second thing to keep identical, and
 * the failure of a copy that drifted would be silent: every write in the fleet
 * refused, on the day a phone updated.
 */
export function lp(buf: Buffer): Buffer {
...
/** Domain separation for the other direction, so no key derived there can collide with one here. */
const REQUEST_KEY_INFO = 'siparu/request-key/v1'

/**
 * A question on its way to the boat, sealed to her inbox.
 *
 * The reverse direction is not a mirror of a frame and should not be built as
 * one. Two things are different, and both follow from who is speaking:
 *
 *   There is no signature. A device holds no signing key by design: its
 *   authority to ask comes from the account session that bought the socket,
 *   not from anything it could sign with. Adding one would create a long-lived
 *   device credential whose only purpose is to be stolen.
 *
 *   There is one recipient, so there is no content key and no per-device wrap.
 *   A frame wraps because one body goes to several screens; a question goes to
 *   one boat. (The specification described this side as carrying "a single
 *   entry in keys". That is one layer with nothing to do: the wrap exists to
 *   let several recipients share a body, and here there are not several. The
 *   layer was left out and the specification says so now.)
 *
 * `id` travels in the clear, and has to. The relay routes the answer back to
 * the socket that asked by that id and counts what is outstanding by it, and a
 * relay that could not read it could not do either. It carries nothing: it is
 * a counter minted by the client for its own bookkeeping.
 *
 * The boat and the id are bound into the derivation rather than carried, so a
 * question captured on the way to one vessel cannot be replayed at another,
 * and the nonce is derived rather than sent for the same reason a frame's wrap
 * nonce is.
 */
export interface SealedRequest {
  /**
   * What kind of message this is, for the party that cannot read the rest of it. The relay
   * sorts a question from a frame by this tag: without one it would have to guess from the
   * shape, and a guess is not something to build routing on.
   */
  type: 'ask'
  v: number
  /** Matches the answer to the question. Cleartext, and read by the relay. */
  id: string
  /** Ephemeral X25519 public key, this request only. */
  eph: string
  /** Ciphertext and tag. */
  body: string
}
...
function requestSecrets(
  ...
  const info = Buffer.from(`${REQUEST_KEY_INFO}/${boat}/${id}`, 'utf8')
  const out = Buffer.from(hkdfSync('sha256', shared, ephPub, info, KEY_BYTES + NONCE_BYTES))
  return { key: out.subarray(0, KEY_BYTES), nonce: out.subarray(KEY_BYTES) }
}

/**
 * Seal one question to a boat's inbox. The device's side, kept here because
 * this file is where the format lives and both ends have to agree byte for
 * byte; the shipping client is the phone.
 */
export function sealRequest(opts: {
  boat: string
  id: string
  /** The request, already serialised. */
  plaintext: string
  /** The boat's raw 32-byte X25519 inbox public key. */
  inbox: Buffer
}): SealedRequest {
  if (opts.inbox.length !== KEY_BYTES) throw new Error('inbox public key must be 32 bytes')
  if (opts.id.length === 0) throw new Error('a request needs an id')
  const eph = generateKeyPairSync('x25519')
  const ephPub = rawPublic(eph.publicKey)
  const shared = diffieHellman({
    privateKey: eph.privateKey,
    publicKey: x25519PublicFromRaw(opts.inbox)
  })
  const { key, nonce } = requestSecrets(shared, ephPub, opts.boat, opts.id)
  return {
    type: 'ask',
    v: FRAME_VERSION,
    id: opts.id,
    eph: b64u(ephPub),
    body: b64u(aead(key, nonce, Buffer.from(opts.plaintext, 'utf8')))
  }
}

/**
 * Open a question addressed to this boat.
 *
 * Throws on anything that is not a question this boat can read, and the caller
 * treats that as "not a request" rather than as a fault: this runs on whatever
 * arrives over a socket, and the relay can no longer look inside to sort it.
 *
 * Opening it proves only that the sender knew this boat's inbox public key,
 * which is public. It is not an authorisation, and nothing downstream may
 * treat it as one: the relay decides who may hold a socket, and every request
 * this yields is still a read against the boat's own store.
 */
export function openRequest(
  ...
  if (typeof envelope !== 'object' || envelope === null) throw new Error('not a request envelope')
  const o = envelope as Record<string, unknown>
  if (o.type !== 'ask') throw new Error('not a question')
  if (o.v !== FRAME_VERSION) throw new Error(`unsupported request version ${String(o.v)}`)
  if (typeof o.id !== 'string' || o.id.length === 0) throw new Error('request id must be a string')
  const ephPub = strictDecode(o.eph, KEY_BYTES)
  const body = strictDecode(o.body)
  const shared = diffieHellman({
    privateKey: x25519PrivateFromRaw(inboxPriv, inboxPub),
    publicKey: x25519PublicFromRaw(ephPub)
  })
  const { key, nonce } = requestSecrets(shared, ephPub, boat, o.id)
  return { id: o.id, plaintext: unaead(key, nonce, body).toString('utf8') }
}
