...
/**
 * Distance per volume, in SI, into the volume per distance a fuel figure is read in.
 *
 * A metre per cubic metre inverted is a cubic metre per metre; the two factors turn that into
 * litres per nautical mile. So 1.65e6 m/m3 is 1.1 L/nm, and unscaled it is the seven-digit
 * number an owner reported seeing on his engine panel.
 */
const M_PER_NM = 1852
const M_PER_M3_TO_L_PER_NM = M_PER_NM * M3_TO_L
...
const SYSTEM_METRIC: Record<string, SystemMetric> = {
  ...
  /**
   * Distance covered per volume burned, which is not in the Signal K schema and is read anyway.
   *
   * The bar this file sets for a key is that the segment pins the reading, and normally the
   * schema is what pins it. There is no `fuel/economy` in the published metadata - only
   * `economyRate`, `averageRate`, `rate`, `used`, `pressure`, `type` - so on the schema's word
   * alone this path would fall through to the unknown-metric fallback and print its raw SI. That
   * is what a boat reported: a seven-digit "1848605.3" in an engine cell.
   *
   * It is claimed on the strength of the calculation that publishes it. Signal K's own
   * derived-data plugin computes `propulsion.<instance>.fuel.economy` as `speed / rate`, metres
   * per second over cubic metres per second, which cancels to metres per cubic metre: a distance
   * over a volume, pinned by the arithmetic rather than by a document. Boats run that plugin, so
   * the path exists in the world whatever the schema says.
   *
   * Read as litres per mile and not as miles per litre, though the SI is the second one and
   * would need no division. A displacement motor yacht burns 20 to 50 L/nm, which reads back as
   * 0.05 to 0.02 nm/L: two digits of leading zeroes on the gauge an owner watches to work out
   * whether he reaches the next fuel dock. The inverse costs a guard and buys a number the whole
   * range of boats can read.
   *
   * Zero is the guard. A boat stopped with an engine running reports rate > 0 at speed 0, the
   * plugin publishes an economy of exactly 0, and the inverse of that is an infinity. There is
   * no economy at rest to print, so the cell says so - the same dot `systemValue` prints for any
   * reading it cannot make - and the graph door answers NaN, which a chart drops as a missing
   * sample instead of drawing a spike to infinity.
   */
  'fuel.economy': {
    sub: true,
    fmt: (v) => (v > 0 ? `${(M_PER_M3_TO_L_PER_NM / v).toFixed(2)} L/nm` : '·'),
    scale: (v) => (v > 0 ? M_PER_M3_TO_L_PER_NM / v : NaN),
    unit: 'L/nm'
  }
}
...
const SYSTEM_SUB: Record<string, string> = {
  ...
  // Not "Fuel economy", which is the word the path uses and the one word this cell cannot have.
  // To a person economy runs the other way - distance per volume - so a cell labelled economy
  // over a figure in litres per mile names the reciprocal of what it shows. The direction is in
  // the name instead, and the unit under it agrees.
  'fuel.economy': 'Fuel per mile',
 ...
 *
 * `fuel.economy` is a different path and is shown: it is a real distance over a real volume, so
 * it has a direction and a name that states it ("Fuel per mile"). The two cannot collide on one
 * screen, because this one never draws a cell at all.
 *
