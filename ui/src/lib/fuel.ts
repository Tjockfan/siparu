 ...
 * (L/h and L/nm) with no way to tell them apart, so `fuel.economyRate` draws no
 * cell and `fuel.economy` draws one that spells its direction out ("Fuel per
 * mile"). Here the person picks the frame by name from a menu, so "Litres / nm"
 * and "nm / litre" each say exactly what they are and cannot be confused for
 * the other.
 */
