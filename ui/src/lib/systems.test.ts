...
import { systemPanels } from "../screens/bridge/useSystems";
...
describe("systemPanels", () => {
  ...
  it("reads a second boat's engine, whose rpm and fuel arrive under different instances", () => {
    // A motorboat running a home-built NMEA 2000 sender: her rpm publishes under `port` and her
    // fuel under `engine`, so one physical engine occupies two columns. Read off her own screen,
    // where the economy cell showed "1848605.3" - the raw metres per cubic metre of a path the
    // Signal K schema does not carry and her derived-data plugin publishes anyway.
    const panels = systemPanels(
      frame({
        "propulsion.port.revolutions": 25.966666666666665,
        "propulsion.engine.fuel.rate": 1.8333333333333333e-6,
        "propulsion.engine.engineLoad": 0.308,
        "propulsion.engine.fuel.economy": 1848605.3,
      }),
    );
    const engine = panels.find((p) => p.key === "engine")!;
    const cell = (path: string) => engine.gauges.find((g) => g.path === path);
    expect(cell("propulsion.port.revolutions")?.value).toBe("1558 rpm");
    expect(cell("propulsion.engine.fuel.rate")?.value).toBe("6.6 L/h");
    expect(cell("propulsion.engine.fuel.economy")?.value).toBe("1.00 L/nm");

    // Two instances, two labels, and the fuel pair sits under the one that reports it. A screen
    // that pivots these onto one row would have to drop a reading to do it.
    expect(cell("propulsion.port.revolutions")?.label).toBe("Port");
    expect(cell("propulsion.engine.fuel.economy")?.label).toBe("Engine");
    expect(cell("propulsion.engine.fuel.economy")?.sub).toBe("Fuel per mile");
    expect(cell("propulsion.engine.fuel.rate")?.sub).toBe("Fuel rate");
  });
