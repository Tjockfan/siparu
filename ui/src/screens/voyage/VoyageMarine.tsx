...
import type { Voyage, VoyageRollup, VoyageStatsCards, TrackPoint, FuelPathsView } from "../../data/api";
...
export default function VoyageMarine() {
  ...
  // The edits exist on her own network only: ashore the read is absent, and so is the button.
  const edits = api.voyage.edits;
  const merge = api.voyage.mergePrevious;
  const undoMerge = api.voyage.undoMerge;
  useEffect(() => {
    if (!edits) return;
    let ok = true;
    edits()
  ...
  // force - a filter naming an engine that has gone quiet has to stay reachable. Never
  // ashore: the choice is a setting of the plugin's, written at her helm.
  const canPick = !!fuelView && fuelSourceOffered(fuelView) && api.config.setFuelPaths !== undefined;
...
function VoyageRow({
  ...
  onToggle: () => void;
  /** Absent where the edit cannot be made - ashore - and then the button is not drawn. */
  onMerge: (() => void) | undefined;
  onUndoMerge: (() => void) | undefined;
  editErr: string | null;
}) {
      ...
      {open && (
        <div className="vy-detail">
          ...
          <div className="vy-meta">
            ...
            {!underway && onMerge && onUndoMerge && (wasJoined || prev) && (
