...
import { useApi, ApiError, type FuelPathsView } from "../../data/api";
...
export default function FuelSourceSheet({
  ...
  const apply = async () => {
    ...
    setErr(null);
    try {
      // The sheet is only offered where the setting can be written (VoyageMarine, canPick).
      if (!api.config.setFuelPaths) throw new Error("this setting is written aboard");
